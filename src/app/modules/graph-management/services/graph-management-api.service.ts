...
export class GraphManagementApiService {
  ...
  getGraphUploadStatus(loaderId: string): Observable<NeptuneLoaderStatusReponse> {
  ...
  uploadGraph(file: any, graphName: string, overwriteExisting: string): Observable<HttpEvent<NeptuneLoaderResponse>> {
    ...
    return this.httpClient.post<NeptuneLoaderResponse>(url, formData, { reportProgress: true, observe: 'events', params: params, headers: headers })
  }
}
