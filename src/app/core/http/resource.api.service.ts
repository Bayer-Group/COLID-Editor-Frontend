...
import { HistoricResourceOverviewDTO, ResourcRevisionHistory } from 'src/app/shared/models/resources/historic-resource-overview-dto';
import { Constants } from 'src/app/shared/constants';
import { LinkType } from 'src/app/shared/models/resources/linking-mapping';
...
export class ResourceApiService {
  ...
  createLink(pidUri: string, linkType: string, pidUriToLink: string, requester: string) : Observable<Resource>{
  ...
  removeLink(pidUri: string, linkType: string, pidUriToUnLink: string, returnTargetResource:boolean, requester: string) : Observable<Resource>{
    const url = environment.colidApiUrl + '/resource/removeLink';
    let params = new HttpParams();
    params = params.append('pidUri', pidUri);
    params = params.append('linkType', linkType);
    params = params.append('pidUriToUnLink', pidUriToUnLink);
    params = params.append('returnTargetResource', returnTargetResource.toString());
    params = params.append('requester', requester);
    return this.httpClient.post<Resource>(url, null, { params });
  }
  ...
  getResourceRevisionHistory(resourcePidUri: string): Observable<ResourcRevisionHistory[]> {
  ...
  deleteResources(resourcePidUris: string[], requester:string): Observable<any> {
  ...
  getResourceHierarchy(resourceType: string[]): Observable<Map<string,string[]>> {
    let params = new HttpParams();
    var url = environment.colidApiUrl + '/resource/getResourceHierarchy'
    return this.httpClient.put<Map<string,string[]>>(url, resourceType,{ params });
  }
}
