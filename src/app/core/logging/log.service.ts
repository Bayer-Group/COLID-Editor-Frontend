...
export class LogService {
    ...
    private static createLogEntry(msg: string, logType: LogType, additionalInfo: any[]): EdmLogEntry {
        ...
        if (additionalInfo != null && additionalInfo.length !== 0) {
            for (const property in additionalInfo) {
                if (additionalInfo.hasOwnProperty(property) && property !== 'ngDebugContext' && !(additionalInfo[property] instanceof Function)) {
    ...
    private writeLogEntry(msg: string, logLevel: LogLevel, ...optionalDetails: any[]) {
        const logEntry = LogService.createLogEntry(msg, LogType.General, optionalDetails);
