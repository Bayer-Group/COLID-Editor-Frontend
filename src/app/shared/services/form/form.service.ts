...
export class FormService {
  ...
  get SelectedConsumerGroup() {
    ...
    const consumerGroupState = this.store.selectSnapshot(state => state).consumerGroups;

    if (consumerGroupState !== null && consumerGroupState.consumerGroups !== null) {
        const selectedConsumerGroup = consumerGroupState.consumerGroups.filter(cg => cg.id === userInfoState.selectedConsumerGroup)[0];
