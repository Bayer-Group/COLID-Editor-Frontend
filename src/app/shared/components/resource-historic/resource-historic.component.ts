...
export class ResourceHistoricComponent implements OnChanges {
  ...
  versionClicked(_: VersionProperty) {
    //do nothing
  }
  ...
  metadataAdditional(entity): Array<MetaDataProperty> {
    if (this.isMetadataMapped) {
      ...
      if (entity.additionals) {
        ...
        var metadatalist = metadatalistvalue.filter(
          (x) => prop.indexOf(x.key) > -1
        );
        return metadatalist;
      }
  ...
  getLastChangeUser(enitiy: HistoryEntity, index): string {
    if (
      enitiy.additions.entity.additionals[Constants.Metadata.HasLastChangeUser]
    ) {
      return enitiy.additions.entity.additionals[
        Constants.Metadata.HasLastChangeUser
      ];
    } else if (
      enitiy.additions.entity.additionals[Constants.Metadata.HasLastReviewer]
    ) {
      return enitiy.additions.entity.additionals[
