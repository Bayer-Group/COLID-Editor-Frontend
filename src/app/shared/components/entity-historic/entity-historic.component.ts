...
export class EntityHistoricComponent implements OnInit, OnDestroy {
  ...
  metadataRelease(entity: Entity): Array<MetaDataProperty> {
    if (this.isMetadataMapped) {
      ...
      const release = entity.properties[Constants.Metadata.MetadataReleaseConfig][0];
  ...
  ngOnInit() {
    this.selectedEntitySubscription = this.selectedEntity.subscribe(id => {
      if (id == null) {
        ...
        this.selectedStep = this.entities.findIndex(entity => entity.id === id);
      }
    })
  }
  ...
  getState(overview: any): string {
    return overview.lifeCycleStatus === Constants.Resource.LifeCycleStatus.Published ? 'published' : 'history';
  }
