...
export class EntityDisplayItemComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.entityProperty) {
      if (this.displayType === 'taxonomy') {
        const range = this.metadataProperty.properties[Constants.Metadata.Range];
  ...
  private fetchTaxonomy(range: string) {
    this.isLoading = true;
    this.store.dispatch(new FetchTaxonomyList(range)).subscribe(res => {
  ...
  nestedMetdata(endpoint: Entity): Array<MetaDataProperty> {
    if (endpoint != null && endpoint.properties) {
      const endpointType = endpoint.properties[Constants.Metadata.EntityType][0];
      const nestedMetdata = this.metadataProperty.nestedMetadata.filter(m => m.key === endpointType)[0]
  ...
  get displayType(): string {
    ...
    if (this.metadataProperty.properties[Constants.Metadata.Range] && this.metadataProperty.properties[Constants.Metadata.NodeKind] === Constants.Metadata.NodeType.IRI && !(group != null && group.key === Constants.Resource.Groups.LinkTypes)) {
      return 'taxonomy';
    }
    ...
    if (this.checkUrl && this.metadataProperty.key === Constants.Metadata.HasTargetUri) {
  ...
  get checkUrl() {
    return this.entityProperty && this.entityProperty.some(value => re_weburl.test(value));
  }

  get checkEmail() {
    return this.entityProperty && this.entityProperty.some(value => re_email.test(value));
  }

  get checkProtocol() {
    return this.entityProperty && this.entityProperty.some(value => re_protocol.test(value));
  }
...
export const re_email = new RegExp(/^([a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)$/);
