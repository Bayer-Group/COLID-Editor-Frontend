...
export class FormExtension {
  ...
  public static createEntityPropertyList(formData: [string, any][], metaDataProperties: MetaDataProperty[]): { [id: string] : any[]; } {
    ...
    for (const property of formData) {
      for (const m of metaDataProperties) {
        ...
        if (property[0] === m.properties[Constants.Metadata.HasPidUri]) {
          let propValue = Array.isArray(property[1]) ? property[1] : [property[1]];
  ...
  private static createPropertyValue(value: any, metadata: MetaDataProperty, isObject: boolean): any {
