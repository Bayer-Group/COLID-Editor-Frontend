...
export class ResourceExtension {
  ...
  public static hasPublishedVersion(resource: Resource) {
    ...
    const lifeCycleStatusProperty = resource.properties[Constants.Metadata.LifeCycleStatus];
    // The entry is published if there is a lifecycle status and it is published or marked for deletion.
    return lifeCycleStatusProperty != null && (lifeCycleStatusProperty[0] === Constants.Resource.LifeCycleStatus.Published || lifeCycleStatusProperty[0] === Constants.Resource.LifeCycleStatus.MarkedDeletion);
  }

  public static hasDraftVersion(resource: Resource) {
    const lifeCycleStatusProperty = resource.properties[Constants.Metadata.LifeCycleStatus];
    if (lifeCycleStatusProperty != null && lifeCycleStatusProperty[0] === Constants.Resource.LifeCycleStatus.Draft) {
      return true;
    }

    const colidEntryDraftProperty = resource.properties[Constants.Metadata.ColidEntryDraft];
    return colidEntryDraftProperty != null && colidEntryDraftProperty[0] != null;
  }
  ...
  public static isAllowedToUnlink(resource: Resource): boolean {
    if (resource.laterVersion || resource.previousVersion) {
      ...
      return !(resource.baseUri != null && ((resource.previousVersion != null && resource.baseUri === resource.previousVersion.baseUri) || (resource.laterVersion != null && resource.baseUri === resource.laterVersion.baseUri)));
    }

    return false;
  }

  public static createHistoricOverviewByResource(resource: Resource): HistoricResourceOverviewDTO {
    return {
      id: resource.id,
      pidUri: resource.pidUri,
      lastChangeDateTime: resource.properties[Constants.Metadata.LastChangeDateTime][0],
      lastChangeUser: resource.properties[Constants.Metadata.HasLastChangeUser][0],
      lifeCycleStatus: resource.properties[Constants.Metadata.LifeCycleStatus][0],
    };
  }
}
