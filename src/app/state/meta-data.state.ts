...
export class MetaDataState {
  ...
  @Selector()
  public static actualSecondMetadata(
    state: MetaDataStateModel
  ): Array<MetaDataProperty> {
    return state.secondMetadata.has(state.secondMetadataType)
      ? state.secondMetadata.get(state.secondMetadataType)
      : null;
  }
  ...
  fetchSecondMetadata(
    ...
    const state = ctx.getState();
    if (
      state.secondMetadataType === action.entityType &&
      state.secondMetadata.has(action.entityType)
    ) {
      return;
    }
  ...
  fetchMetadataRelease(
    ...
    if (
      state.metadataType === action.entityType &&
      state.metadata.has(action.metadataRelease)
    ) {
      ctx.patchState({
    ...
    this.metaDataApiService
      .getMetadataRelease(action.metadataRelease, action.entityType)
      .subscribe((res) => {
        const state = ctx.getState();
  ...
  fetchLinkResourcesTypesMetadata(
    ...
    this.resourceApiService.getResourceHierarchy(type).subscribe((x) => {
      const state = ctx.getState();
