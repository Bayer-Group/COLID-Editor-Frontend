...
export class MessageTemplateEditComponent implements OnInit {
  ...
  onPlaceholder(placeholder: string) {
    if (this.activeControl != null) {
      this.activeControl.patchValue(`${this.activeControl.value} ${placeholder} `);
    }
  }

  get valuesChanged() {
    const sameSubject = this.templateForm.controls['subjectControl'].value.trim() !== this.messageTemplate.subject;
    const sameBody = this.templateForm.controls['bodyControl'].value.trim() !== this.messageTemplate.body;
  ...
  onReset() {
    if (this.valuesChanged) {
      this.fillFormControl();
      this.snackbar.info('Template \'' + this.messageTemplate.type + '\' restored', null);
    }
  }

  onSubmit() {
    if (!this.valuesChanged) {
      this.snackbar.warning('Template unchanged', 'The template hasn\'t been changed.');
    } else {
      if (this.valuesValid) {
        this.snackbar.warning('Template empty', 'The template contains empty fields.');
        return;
      }
      this.messageTemplate.subject = this.templateForm.controls['subjectControl'].value.trim();
      this.messageTemplate.body = this.templateForm.controls['bodyControl'].value.trim();
  ...
  updateMessageTemplate() {
    this.store.dispatch(new UpdateMessageTemplate(this.messageTemplate))
      .subscribe(res => this.snackbar.success('Template \'' + this.messageTemplate.type + '\' saved', null));
  }
