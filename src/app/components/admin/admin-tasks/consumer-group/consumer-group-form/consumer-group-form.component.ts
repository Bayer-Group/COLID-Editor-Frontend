...
import { ConsumerGroupState, FetchConsumerGroupMetadata, FetchConsumerGroupDetails, DeleteConsumerGroup, SetConsumerGroup } from 'src/app/state/consumer-group.state';
...
export class ConsumerGroupFormComponent implements OnInit, OnDestroy {
  @Select(ConsumerGroupState.getConsumerGroup) consumerGroup$: Observable<ConsumerGroupResultDTO>;
  @Select(ConsumerGroupState.getConsumerGroupMetadata) consumerGrouppMetadata$: Observable<Array<MetaDataProperty>>;
  ...
  deletionText = 'You are about to delete a consumer group. <br>' +
  ...
  get isActive() {
    return this.consumerGroup != null && this.consumerGroup.lifecycleStatus === Constants.ConsumerGroup.LifecycleStatus.Active;
  }

  get isDeprecated() {
    return this.consumerGroup != null && this.consumerGroup.lifecycleStatus === Constants.ConsumerGroup.LifecycleStatus.Deprecated;
  }

  constructor(private store: Store, private route: ActivatedRoute, private snackBar: ColidMatSnackBarService, private router: Router, private consumerGroupService: ConsumerGroupApiService) { }
  ...
  setPlaceholder() {
    this.placeholder[Constants.ConsumerGroup.HasConsumerGroupLifecycleStatus] = Constants.ConsumerGroup.LifecycleStatus.Active;
  }
  ...
  fetchingEntityDetails() {
    this.store.dispatch(new FetchConsumerGroupDetails(this.entityId)).subscribe();
  }
