...
import { PidUriTemplateState, DeletePidUriTemplate, CreatePidUriTemplate, EditPidUriTemplate, ReactivatePidUriTemplate } from 'src/app/state/pid-uri-template.state';
...
export class PidUriTemplateTableComponent implements OnInit, OnDestroy {
  @Select(PidUriTemplateState.getPidUriTemplate) pidUriTemplate$: Observable<PidUriTemplateResultDTO>;
  ...
  ngOnInit() {
    this.setPlaceholder();
    this.pidUriTemplateMetadataSubscription = this.pidUriTemplateMetadata$.subscribe(res => {
      if (res) {
        this.metaData = res.filter(r => r.properties[this.pidUriConstant] !== Constants.Metadata.EntityType)
          .sort((a, b) => a.properties[Constants.Metadata.Order] - b.properties[Constants.Metadata.Order]);
      }
      if (this.metaData) {
        this.buildForm();
      }
  ...
  buildForm(): void {
    ...
    for (const m of this.metaData) {
      formBuilderGroup[m.properties[this.pidUriConstant]] = [m.properties[this.pidUriConstant]];
    }
  ...
  showValidationResult(validationResult: ValidationResult) {
    validationResult.results.forEach((result) => {
      this.pidUriTemplateForm.controls[result.path].setErrors({ incorrect: true, result: result });
    });
  }
