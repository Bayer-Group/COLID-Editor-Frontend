...
export class FormItemInputAttachmentComponent extends FormItemInputBaseComponent implements OnInit {
  ...
  get attachmentMetadata() {
    if (this.internalValue != null && this.internalValue.properties != null) {
      const metadata = this.metaData.find(r => r.key === Constants.Attachment.Type);
  ...
  writeValue(value: Entity): void {
    if (value != null) {
      ...
      let attachmentFileName = this.internalValue.properties[Constants.Metadata.RdfsLabel];
  ...
  confirmAndRemoveAttachment() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.attachmentApiService.deleteAttachment(this.internalValue.id).subscribe(
          deletionResult => {
            this.removeFormItem.emit();
            this.snackBar.success(this.label, `The ${this.label.toLowerCase()} has been deleted completely.`);
          },
          error => {
            if(error.status == 409) {
              this.removeFormItem.emit();
              this.snackBar.info(this.label, `The ${this.label.toLowerCase()} is already in use by the published or another resource and can therefore not be deleted completely. However, the reference has been removed.`, null, 8000);
            }
          });
      }
    });
  ...
  handleAttachmentFormChanged(event: FormChangedDTO) {
    ...
    const entity = this.createEntity(event.formValue, this.internalValue.id, this.attachmentMetadata);
  ...
  uploadAttachment() {
    ...
    if(this.attachmentFileToUpload == null || comment == null) {
      this.snackBar.error(this.label, `Please select a file and add a comment before uploading the ${this.label.toLowerCase()}.`);
    } else {
      this.isUploadInProgress = true;
      this.attachmentApiService.uploadAttachment(this.attachmentFileToUpload, comment).subscribe(
        result => {
          this.isUploadInProgress = false;
          const oldTemporaryId = this.internalValue.id;

          this.internalValue.id =result.id
          this.internalValue.properties[Constants.Metadata.EntityType] = [Constants.Attachment.Type];
          this.internalValue.properties[Constants.Metadata.RdfsLabel] = [result.s3File.fileName];
          this.internalValue.properties[Constants.Attachment.FileSize] = [result.s3File.fileSize];
          this.internalValue.properties[Constants.Attachment.FileType] = [result.s3File.fileType];
    
          const entity = this.createEntity(this.internalValue.properties, this.internalValue.id, this.attachmentMetadata);
          const formItemChangedDTO = new FormItemChangedDTO(Constants.Metadata.RdfsLabel+this.indexerNested, entity);
          this.handleInputChange(formItemChangedDTO);

          this.snackBar.success(this.label, `The ${this.label.toLowerCase()} was uploaded successfully.`);
          this.attachmentUploaded.emit(new AttachmentUploadedDto(oldTemporaryId, result.id));
        },
        error => {
          this.isUploadInProgress = false;
          this.snackBar.error(this.label, `The ${this.label.toLowerCase()} could not be uploaded.`);
        });
    }
  } 
  ...
  get isFileUploaded() : boolean {
    return this.internalValue == null ? false : !this.internalValue.id.startsWith(Constants.Resource.Prefix);
  }

  get isNewEntity() : boolean {
    return this.internalValue == null ? false : this.newAttachmentEntities.includes(this.internalValue.id);
  }
}
