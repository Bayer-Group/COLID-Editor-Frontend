  ...
  filter(key: string = null) {
    ...
    const resourceSearchObject: ResourceSearchDTO = {
      ...
      searchText:
        ...
        this.filters.searchText.split('"').length - 1 == 1
          ? this.filters.searchText.replace('"', "")
          : this.filters.searchText,
      lastChangeUser: this.filters.lastChanged ? this.userEmail : null,
      consumerGroup: this.filters.consumerGroup
        ? this.store.selectSnapshot(UserInfoState.getSelectedConsumerGroupId)
        : null,
    ...
    this.logger.info("PID_SIDEBAR_FILTER_RESOURCE_SEARCHED", {
