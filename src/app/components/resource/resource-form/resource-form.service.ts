...
export class ResourceFormService {
  ...
  createResourceDto(
    ...
    newResource = this.transformDistributionEndpointList(
      newResource,
      mainDistribution
    );
  ...
  transformAttachmentEntitiesToIdList(
    resource: ResourceRequestDTO
  ): ResourceRequestDTO {
    let attachments = resource.properties[Constants.Metadata.HasAttachment];

    if (attachments != null && attachments.length > 0) {
      let uploadedAttachments = attachments.filter(
        (attachment) => !attachment.id.startsWith(Constants.Resource.Prefix)
      );

      resource.properties[Constants.Metadata.HasAttachment] =
        uploadedAttachments;
    }
  ...
  transformDistributionEndpointList(
    resource: ResourceRequestDTO,
    mainDistribution: string
  ): ResourceRequestDTO {
    var distributionEndpoints =
  ...
  get SelectedConsumerGroup() {
    const userInfoState: UserInfoStateModel = this.store.selectSnapshot(
      (state) => state
    ).UserInfo;

    if (
      userInfoState !== null &&
      userInfoState.selectedConsumerGroupId !== null
    ) {
  ...
  get DefaultPidUriTemplateKey() {
    ...
    if (
      userInfoState &&
      userInfoState.consumerGroups &&
      userInfoState.selectedConsumerGroupId
    ) {
      const selectedConsumerGroup: ConsumerGroupResultDTO =
        userInfoState.consumerGroups.filter(
          (cg) => cg.id === userInfoState.selectedConsumerGroupId
        )[0];
      if (selectedConsumerGroup) {
        const defaultPidUriTemplateKeys =
          selectedConsumerGroup.properties[
            Constants.ConsumerGroup.HasDefaultPidUriTemplate
          ];
  ...
  extractAvailablePidUriTemplates(
    ...
    const selectedConsumerGroup: ConsumerGroupResultDTO = consumerGroups.filter(
    ...
    const availablePidUriTemplateKeys =
      selectedConsumerGroup.properties[
    ...
    const pidUriTemplateNames: Array<PidUriTemplateResultDTO> =
      new Array<PidUriTemplateResultDTO>();

    for (const pidUriTemplateKey of availablePidUriTemplateKeys) {
      const pidUriTemplate = pidUriTemplates.filter(
        (put) => put.id === pidUriTemplateKey
      )[0];
