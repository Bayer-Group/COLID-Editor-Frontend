...
export class ResourceDisplayItemDistributionComponent {
  @Select(ResourceState.getActiveMainDistribution)
  mainDistribution$: Observable<string>;
  ...
  getTypeMetadata(properties: { [id: string]: any[] }) {
    ...
    prop.properties = prop.properties.filter(
      (distProp) =>
        distProp.properties[Constants.Metadata.HasPidUri] !==
