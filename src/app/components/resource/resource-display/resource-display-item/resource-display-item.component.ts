...
export class ResourceDisplayItemComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (metadataKey === this.pidUriConstant || metadataKey === MetaDataPropertyIdentifier.baseUri) {
  ...
  getResourcePropertyValue() {
    if (this.resource.properties) {
      const value = this.resource.properties[this.metadata.properties[this.pidUriConstant]];
