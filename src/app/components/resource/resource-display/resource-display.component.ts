...
export class ResourceDisplayComponent implements OnInit, OnDestroy {
  ...
  tryGetMetadata() {
    if (this.activeResource && this.metadataMap) {
      const release = this.activeResource.properties[Constants.Metadata.MetadataReleaseConfig][0];
      this.metadata = this.metadataMap.has(release) ? this.metadataMap.get(release) : [];
    }
  }
}
